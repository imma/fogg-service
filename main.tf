...
resource "aws_subnet" "service" {
  ...
  cidr_block              = "${cidrsubnet(data.aws_vpc.current.cidr_block,var.service_bits,element(concat(split(" ",lookup(data.external.org.result,"service_${data.terraform_remote_state.app.app_name}_${var.service_name}","")),split(" ",lookup(data.external.org.result,"service_${var.service_name}",""))),count.index))}"
...
resource "aws_subnet" "service_v6" {
  ...
  ipv6_cidr_block                 = "${cidrsubnet(data.aws_vpc.current.ipv6_cidr_block,8,element(concat(split(" ",lookup(data.external.org.result,"service_v6_${data.terraform_remote_state.app.app_name}_${var.service_name}","")),split(" ",lookup(data.external.org.result,"service_v6_${var.service_name}",""))),count.index))}"
...
resource "aws_instance" "service" {
  ...
  subnet_id                   = "${element(concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.public_network ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets)),count.index)}"
...
resource "aws_elb" "service" {
  ...
  subnets = ["${split(" ",var.public_lb ? join(" ",data.terraform_remote_state.env.public_subnets) : join(" ",concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.public_network ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets))))}"]
...
resource "aws_autoscaling_group" "service" {
  ...
  vpc_zone_identifier  = ["${concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.public_network ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets))}"]
...
module "efs" {
  ...
  subnets  = ["${concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.public_network ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets))}"]
