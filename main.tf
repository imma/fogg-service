...
resource "aws_instance" "service" {
  ...
  subnet_id                   = "${element(concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.cond ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets)),count.index)}"
...
resource "aws_elb" "service" {
  ...
  subnets = ["${split(" ",var.public_lb ? join(" ",data.terraform_remote_state.env.public_subnets) : join(" ",concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.cond ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets))))}"]
...
resource "aws_autoscaling_group" "service" {
  ...
  vpc_zone_identifier  = ["${concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.cond ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets))}"]
...
module "efs" {
  ...
  subnets  = ["${concat(aws_subnet.service.*.id,aws_subnet.service_v6.*.id,formatlist(var.cond ? "%[1]s" : "%[2]s",data.terraform_remote_state.env.public_subnets,data.terraform_remote_state.env.common_subnets))}"]
